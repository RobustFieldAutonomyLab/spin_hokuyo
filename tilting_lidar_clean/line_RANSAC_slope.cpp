   ...
   for (int i = 0; i < 50; i++)
   {
    ...
    // }
     ...
     for(int n = 0; n < 921; n++)
     {
        ...
        {
          m_test = tan((n * msg.angle_increment) + msg.angle_min) - b / (msg.ranges[n] * cos((n * msg.angle_increment) + msg.angle_min));

          if (m_test < m + threshold && m > m - threshold && m_test != inf && m_test != -inf && m_test != NAN && m_test != -NAN)
          {
             total_good++;
          }
        }
